...
namespace com.ootii.Cameras
{
    ...
    public class OrbitRig : BaseCameraRig
    {
        ...
        public int mSmoothingValue = 5;
        public int SmoothingValue
        {
            get { return mSmoothingValue; }
            set { mSmoothingValue = value; }
        }
        ...
        public override void RigLateUpdate(float rDeltaTime, int rUpdateIndex)
        {
            ...
            //WallCorrection(_Anchor.position, ref lNewCameraPosition);

            // Set the values
            _Transform.position = Vector3.Lerp(_Transform.position, lNewCameraPosition, Time.deltaTime * mSmoothingValue);
        ...
        private void WallCorrection(Vector3 rFromObject, ref Vector3 rToTarget)
        {
            RaycastHit lWallHit = new RaycastHit();
            if (UnityEngine.Physics.Linecast(rFromObject, rToTarget, out lWallHit))
            {
                rToTarget = new Vector3(lWallHit.point.x, rToTarget.y, lWallHit.point.z);
            }
        }
    }
}
